...
const Settings: React.FC = () => {
   ...
   const fetchIntegrations = async () => {
      ...
      try {
         ...
         // Ensure OpenRouter exists in the list
         if (!data.find(i => i.id === 'openrouter')) {
            const openRouterConfig: IntegrationConfig = {
               id: 'openrouter',
               name: 'OpenRouter AI',
               enabled: false,
               apiKey: '',
               webhookUrl: '',
               settings: { serverRegion: 'GLOBAL' }
            };
            await ApiService.integrations.saveConfig(openRouterConfig);
            data.push(openRouterConfig);
         }
   ...
   const handleProviderChange = async (provider: 'google' | 'openrouter') => {
      try {
         await updateConfig({ aiProvider: provider, aiModel: provider === 'google' ? 'flash' : 'z-ai/glm-4.5-air:free' });
         addNotification({ title: 'Kernel IA', message: `Fournisseur basculé sur ${provider === 'google' ? 'Google DeepMind' : 'OpenRouter'}.`, type: 'success' });
      } catch (err) {
         addNotification({ title: 'Erreur', message: 'Échec du changement de fournisseur.', type: 'error' });
      }
   };
   ...
   const handleModelChange = async (model: string) => {
      try {
         ...
         addNotification({ title: 'Kernel IA', message: `Modèle mis à jour : ${model}.`, type: 'success' });
      } catch (err) {
            ...
            <div className="w-full lg:w-72 flex flex-col gap-1 shrink-0 bg-white border border-[#ededed] p-2 rounded-xl shadow-sm">
               {tabs.map(tab => (
                  <button
                     ...
                     className={`flex items-center gap-4 px-4 py-3 rounded-lg transition-all text-left group relative ${activeTab === tab.id
                        ...
                        <div className="space-y-8">
                           ...
                           <div className="p-6 bg-[#f8f9fa] border border-[#ededed] rounded-2xl">
                              <div className="flex items-center justify-between mb-6">
                                 <h3 className="text-xs font-black text-[#1c1c1c] uppercase tracking-widest">Fournisseur d'Intelligence</h3>
                              ...
                              <div className="flex gap-4 mb-6">
                                 <button
                                    ...
                                    className={`flex-1 py-3 rounded-xl border-2 transition-all font-bold text-sm ${config.aiProvider !== 'openrouter' ? 'border-[#3ecf8e] bg-white text-[#1c1c1c] shadow-sm' : 'border-transparent bg-white/50 text-[#9ca3af]'}`}
                                 >
                                    Google Gemini
                                 </button>
                                 <button
                              ...
                              {(!config.aiProvider || config.aiProvider === 'google') ? (
                                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    >
                                       ...
                                       <p className="text-[11px] text-[#686868] font-medium leading-relaxed">Rapidité optimisée. Idéal pour le chat client.</p>
                                    </button>
                                    <button
                                    ...
                                    >
                                       ...
                                       <p className="text-[11px] text-[#686868] font-medium leading-relaxed">Puissance maximale. Idéal pour l'analyse.</p>
                                    </button>
                                 </div>
                              ) : (
                                 <div className="space-y-4">
                                    <label className="text-[10px] font-black text-[#686868] uppercase tracking-widest">Modèle OpenRouter (ID)</label>
                                    <div className="flex gap-2">
                                       <input
                                          type="text"
                                          value={config.aiModel || ''}
                                          onChange={(e) => handleModelChange(e.target.value)}
                                          className="flex-1 h-11 border border-[#ededed] rounded-lg px-4 font-bold text-sm"
                                          placeholder="ex: openai/gpt-4o"
                                       />
                                       <div className="flex gap-2">
                                          <button onClick={() => handleModelChange('z-ai/glm-4.5-air:free')} className="px-3 bg-[#f0fdf4] text-[#3ecf8e] border border-[#dcfce7] rounded-lg text-[10px] font-black hover:bg-[#dcfce7]">GLM-4 (Gratuit)</button>
                                          <button onClick={() => handleModelChange('openai/gpt-4o')} className="px-3 bg-white border border-[#ededed] rounded-lg text-[10px] font-black hover:bg-gray-50">GPT-4o</button>
                                          <button onClick={() => handleModelChange('anthropic/claude-3-opus')} className="px-3 bg-white border border-[#ededed] rounded-lg text-[10px] font-black hover:bg-gray-50">Claude 3</button>
                                       </div>
                                    </div>
                                    <p className="text-[10px] text-[#9ca3af] italic">Assurez-vous d'avoir configuré votre clé API OpenRouter dans l'onglet Gateway.</p>
                                 </div>
                              )}
                           </div>
